    ...
    handleRemote: function(respondwith, req, res) {
        ...
        var options = {
            ...
            headers: extend({}, req.headers, {
                host: url.parse(respondwith).hostname,
                'accept-encoding': '*'
            })
        }
    ...
    handleLocalFile: function(respondwith, req, res) {
        ...
        if(fileExists(resFilePath)) {
            if(path.extname(respondwith) === '.js' || path.extname(respondwith) === '.json') {
                ...
                if(path.extname(respondwith) === '.json') {
                    respond = JSON5.parse(fs.readFileSync(resFilePath, 'utf-8'));
                } else {
                    respond = requireUncached(resFilePath);
                }
